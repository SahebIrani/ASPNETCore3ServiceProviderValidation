...
namespace Simple.Controllers
{
    public class MyControllers : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<MyEntity>>> GetAsync(CancellationToken cancellationToken = default)
        {
            IEnumerable<MyEntity> result = await Service.GetMyEntities(cancellationToken);
            return Ok(result);
        }

        public ActionResult<IEnumerable<MyEntity>> FakeEntites()
        {
